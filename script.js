const Memory = {
    ...
    refresh: () => {
        ...
        const meetTime = Memory.get('spirit_meet');
        const days = Math.floor((Date.now() - meetTime) / 86400000) + 1;
        document.getElementById('badge-ui').innerHTML = `${n}的陪伴者<br>陪伴 ${days} 天 | 星星 ${Memory.get('stars') || 0}`;
    }
};
...
function switchMode(mode, el) {
    ...
    dom.vent.style.display = 'none'; dom.bubble.style.opacity = 1; dom.eyes.forEach(e => e.className = 'eye'); 
